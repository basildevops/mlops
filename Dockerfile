...
RUN python3 -m venv .venv && source .venv/bin/activate && python3 -m pip install numpy pandas matplotlib scikit-learn mlflow

# Train, Track the Model, Create an experiment 
RUN python3 app/simple_ml_mlflow.py

# Now set the default command to deploy your model and serve it on port 5000
CMD source .venv/bin/activate && mlflow models serve -m runs:/${RUN_ID}/linear_regression_model 
